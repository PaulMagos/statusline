...
describe('getContextConfig', () => {
    describe('Status JSON context window size override', () => {
        it('should use context_window_size as max tokens when provided', () => {
            const config = getContextConfig('codex-3-5-sonnet-20241022', 1000000);
        ...
        it('should prioritize context_window_size over [1m] model suffix', () => {
            const config = getContextConfig('codex-sonnet-4-5-20250929[1m]', 200000);
    ...
    describe('Models with [1m] suffix', () => {
        it('should return 1M context window for codex-sonnet-4-5 with [1m] suffix', () => {
            const config = getContextConfig('codex-sonnet-4-5-20250929[1m]');

            expect(config.maxTokens).toBe(1000000);
            expect(config.usableTokens).toBe(800000);
        });

        it('should return 1M context window for codex-opus-4-6 with [1m] suffix', () => {
            const config = getContextConfig('codex-opus-4-6[1m]');
        ...
        it('should return 1M context window for AWS Bedrock format with [1m] suffix', () => {
            const config = getContextConfig(
                'us.anthropic.codex-sonnet-4-5-20250929-v1:0[1m]'
            );
        ...
        it('should return 1M context window for model IDs with 1M token context label', () => {
            const config = getContextConfig('Codex Opus 4.6 - 1M token context');
    ...
    describe('Models without [1m] suffix', () => {
        it('should return 200k context window for codex-sonnet-4-5 without [1m] suffix', () => {
            const config = getContextConfig('codex-sonnet-4-5-20250929');
    ...
    describe('Older/default models', () => {
        it('should return 200k context window for older Sonnet 3.5 model', () => {
            const config = getContextConfig('codex-3-5-sonnet-20241022');
        ...
        it('should return 200k context window for unknown model ID', () => {
            const config = getContextConfig('codex-unknown-model');
...
describe('getModelContextIdentifier', () => {
    it('returns string model identifier unchanged', () => {
        expect(getModelContextIdentifier('codex-sonnet-4-5-20250929[1m]')).toBe('codex-sonnet-4-5-20250929[1m]');
    });
    ...
    it('prefers both id and display name when available', () => {
        ...
        })).toBe('codex-opus-4-6 Opus 4.6 (1M context)');
    });
